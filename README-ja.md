...
AI CLI が注意を引くべきタイミングで、音とバナーで通知する CLI ツールです。対応 CLI にはネイティブな Hook/notify をインストールでき、現時点では Codex をサポートしています。従来の PTY ラッパーモードも残しています。
...
まず一度だけ Codex 用の notify コマンドを設定します。

```sh
crai install codex
```

これで `~/.codex/config.toml` に `notify` が書き込まれます。以後は普通に `codex` を使うだけで、Codex の通知イベント時に以下が呼ばれます。
